...
describe('auxiliary methods', () => {
    it('isScreenshotCommand', () => {
        const reporter = new AllureReporter({ stdout: true })
        expect(reporter.isScreenshotCommand({ endpoint: '/session/id/screenshot' } as WDIOReporter.Command)).toEqual(true)
        expect(reporter.isScreenshotCommand({ endpoint: '/wdu/hub/session/id/screenshot' } as WDIOReporter.Command)).toEqual(true)
        expect(reporter.isScreenshotCommand({ endpoint: '/session/id/click' } as WDIOReporter.Command)).toEqual(false)
        expect(reporter.isScreenshotCommand({ command: 'takeScreenshot' } as WDIOReporter.Command)).toEqual(true)
        expect(reporter.isScreenshotCommand({ command: 'elementClick' } as WDIOReporter.Command)).toEqual(false)
        expect(reporter.isScreenshotCommand({ endpoint: '/session/id/element/id/screenshot' } as WDIOReporter.Command)).toEqual(true)
    })
    ...
    it('should populate the correct deviceName', () => {
        ...
        reporter.onTestStart({ cid: '0-0', title: 'SomeTest' } as WDIOReporter.Test)
...
describe('nested suite naming', () => {
    it('should not end test if no hook ignored', () => {
        ...
        reporter.onSuiteStart({ title: 'bar' } as WDIOReporter.Suite)
